...
AREXPORT bool ArSocket::close()
{
  ...
  if (myFD != INVALID_SOCKET) {
    if (!myHost.empty() && (myPort != -1)) {
      ArLog::log(ArLog::Verbose, "Closing %s socket %s:%i (0x%x)",
                 toString(myType),
                 myHost.c_str(),
                 myPort,
                 myFD);
      ...
      ArLog::log(ArLog::Verbose, "Closing %s socket (0x%x)",
                 toString(myType),
                 myFD);
    }
  }
  ...
  if (myDoClose && (myFD != INVALID_SOCKET))
  {
    ...
    if (shutdownRet != 0) {
      ...
      ArLog::log(ArLog::Normal, "Shutdown %s socket (0x%x) returns %i (error = %i)",
                 toString(myType),
                 myFD,
                 shutdownRet, 
                 error);
    ...
    if (closeRet  != 0) {
      int error = WSAGetLastError();
      ArLog::log(ArLog::Normal, "Close %s socket (0x%x) returns %i (error = %i)",
                 toString(myType),
                 myHost.c_str(),
                 closeRet,
                 error);
