...
AREXPORT void ArModeTeleop::help(void)
{
  ...
  ArLog::log(ArLog::Terse, "%13s:  turn left by %d", "left arrow");
  ArLog::log(ArLog::Terse, "%13s:  turn right by %d", "right arrow");
  if (myRobot->hasLatVel())
  {
    ArLog::log(ArLog::Terse, "%13s:  move left by %d", "z");
    ArLog::log(ArLog::Terse, "%13s:  move right by %d", "x");
  }
