				      ...
				      size_t errorBufferLen)
{
  ...
  else if (param->getType() == ArConfigArg::CPPSTRING)
  {
    if(param->setCppString(arg->getFullString()))
    {
      IFDEBUG(ArLog::log(ArLog::Verbose, "%sSet parameter cppstring '%s' to '%s'",
                          myLogPrefix.c_str(),
		                      param->getName(), param->getCppString().c_str()));
	    continue;
	  }
	  else
	  {
      ArLog::log(ArLog::Verbose, "%sCould not set cppstring parameter '%s' to '%s'",
           myLogPrefix.c_str(),
		       param->getName(), param->getCppString().c_str());
	    if (errorBuffer != NULL && errorBuffer[0] == '\0')
	      snprintf(errorBuffer, errorBufferLen, "%s could not be set to '%s'.", param->getName(), arg->getFullString());
	...
	{
    ArLog::log(ArLog::Terse, "%sArConfig::parseArgument: Don't know how to handle type %d of argument '%s' in section, got string '%s', in section '%s'.", 
      param->getType(), myLogPrefix.c_str(), arg->getExtraString(), arg->getFullString(), mySection.c_str());
	}
      }
    }
  }
				     ...
				     bool writePriorities)
{
  ...
  for (paramIt = params->begin(); paramIt != params->end(); paramIt++)
  {
    ...
    else if (param->getType() == ArConfigArg::STRING || param->getType() == ArConfigArg::CPPSTRING)
    {
    ...
    {
      ArLog::log(ArLog::Terse, "%swriteSection: unrecognized data type for %s", myLogPrefix.c_str(), param->getName());
    }
